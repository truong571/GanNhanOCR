...
Ngày 22/09/2026 · HEAD 4e0a314bca (chưa commit) · mọi số ở đây sinh bởi script, 0 token LLM, 0 API, **0 hộp GT người**.
...
Mốc v1 trên val (img 1024, thr 0,15): litho ok50 **95,0 %**, miss 2,1 %, extra 2,6, tầng n==N **77,0 %**, cắt 7,1 %; STT F1 **0,876**.
Lưu best theo ok50_litho **với điều kiện** F1_STT ≥ F1_STT(v1) − 0,01; **dừng** khi 3 epoch không tăng ≥ 0,2 điểm hoặc STT tụt.
Nghiệm thu cuối (không phải val): (i) `scripts/measure/detector_transfer.py` với `NOM_DETECTOR_CKPT=<v2>`: STT đối chứng
`stt_control_pct_cols_eq_pipeline_cfg` ≥ 90,1 không giảm; (ii) `box_ref_eval.py` trên 27 trang/sách: I5 cột, ok50, và **chỉ số cuối là
"cắt vào thân chữ" giảm**; (iii) build `--limit 10` với v2 + `box_decoder: pitch`: tỉ lệ `detector_low`/`ink_cut` giảm, F1 cross-col không tăng.
...
- Với phương án B: kỳ vọng hợp lý là tầng n == N thô 77 → ≥ 90 % (mức STT), ok50 ≥ 98 %, cắt vào thân chữ ≤ ref + 2 điểm, STT F1 không giảm.
  Không hứa hơn: nhãn yếu chia đều-theo-mực có sai số hệ thống ở chữ ⿱ và chữ nhạt.
