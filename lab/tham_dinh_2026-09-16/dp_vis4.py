"""Kết hợp: CALIB + đệ quy neo ngữ liệu (+corpus≥2, loại-một-trang) + phát xạ ảnh λ=0,25."""
...
from thuc_nghiem import load_cols, perturb, qn, sim, PROD, CALIB, set_matrix, aa, Corpus
...
def dp_align(chars, syls, crop_rows, lam, gap_vis, M, pg, corpus):
    ...
    def tcost(i, j):
        ...
        if corpus and len(C.pair_pages.get((chars[i], syls[j].lower()), set()) - {pg}) >= 2:
            return min(base, 0.0)
        return base
    INF = float("inf")
    ...
    for i in range(m + 1):
        for j in range(n + 1):
            ...
            if i > 0 and j > 0 and dp[i-1][j-1] < INF:
                c = dp[i-1][j-1] + tcost(i-1, j-1) + lam * V[i-1, j-1]
...
def run(eq, kind, M, lam, gv, corpus, seed=2026):
    ...
    for col in eq:
        ...
        pairs, dels, inss = dp_align(c2, s2, crop_rows_for(kind, k, len(c2), rows), lam, gv, M, (col["book"], col["page"]), corpus)
        ncol += 1
...
print(f"test+val: {len(eq)} cột m=n≥10 · ô {sum(len(c['syl']) for c in eq)}")
for kind in ("drop_char", "drop_syl", "none"):
    out = []
    for name, lam, corpus in (("CALIB thuần", 0, False), ("CALIB+corpus≥2", 0, True), ("CALIB+ảnh λ0.25", 0.25, False), ("CALIB+corpus+ảnh λ0.25", 0.25, True)):
        n, w, g, nc = run(eq, kind, CALIB, lam, 8.0, corpus)
