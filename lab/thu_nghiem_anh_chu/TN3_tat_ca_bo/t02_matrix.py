...
MX.to_csv(HERE / 'matrix.csv', index=False)
np.save(BIG / 'boot_img_sums.npy', BI)
...
CM.to_pickle(BIG / 'cells_masks.pkl')
...
SUM = dict(key='TN3_tat_ca_bo', date='2026-09-26', api_calls=0, seed=SEED, B=dict(img=ARGS.B_img, ihr=ARGS.B_ihr, tk=ARGS.B_tk),
           thresholds=dict(C5_T=C5T, C5_L=C5L, C2_T=C2T, C2_L=C2L, stt_q=0.00015, tn2_theta=json.load(open(HERE / 't01_summary.json'))['theta_used']),
...
json.dump(SUM, open(HERE / 't02_summary.json', 'w'), ensure_ascii=False, indent=1, default=float)
