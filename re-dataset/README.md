...
**54,156 nhãn cấp KÝ TỰ** + **10,369 chú giải cấp ÂM TIẾT**
= 64,525 dòng · 1,660 lớp chữ · 447 trang · 4,011 cột

> ⚠️ **Đừng phát biểu là "64,525 nhãn".** 10,369 dòng tầng SYLLABLE có cột
> `label` **rỗng** — chúng chỉ ghi âm Quốc ngữ, KHÔNG gán chữ Nôm. Con số đem so với các bộ
> dữ liệu Hán Nôm khác là **54,156**.
...
| `usable_image` | `0` = ảnh trắng hoặc bị cắt mất nét (481 ô). Nhãn có thể vẫn đúng; đừng chấm chiều ảnh ở các ô này |
...
| `page_cot_lech` | `1` = trang này không đủ 9 cột (207 ô / 3 trang). Ghép cột Nôm↔Quốc ngữ có thể đã trượt |
...
**Hệ quả phải biết:** 175 ô có lớp chữ **không xuất hiện trong
...
Khi đánh giá **cấp ký tự**, bỏ các ô `label_in_train == 0` — nếu không, 175 ô đó
...
> luôn 10,369 dòng SYLLABLE có ô rỗng, tức 10,544 dòng
> chứ không phải 175. Lọc trong phạm vi `label_level == "char"`.
