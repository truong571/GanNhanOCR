...
def main():
    ...
    # Dictionary
    try:
        from label_characters import load_translation_dict
        dict_dir = base / "Alignment" / "Code" / "dict"
        dict_path = dict_dir / "QuocNgu_SinoNom_Merged.csv"
        if not dict_path.exists():
            dict_path = dict_dir / "QuocNgu_SinoNom_TongHop3.csv"
        trans_dict = load_translation_dict(str(dict_path))
